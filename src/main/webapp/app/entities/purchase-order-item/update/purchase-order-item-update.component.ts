...
import {
  ...
  PurchaseOrderItemFormService,
} from "./purchase-order-item-form.service";
...
export class PurchaseOrderItemUpdateComponent implements OnInit {
  ...
  editForm: PurchaseOrderItemFormGroup =
    this.purchaseOrderItemFormService.createPurchaseOrderItemFormGroup();

  comparePurchaseOrder = (
    o1: IPurchaseOrder | null,
    o2: IPurchaseOrder | null,
  ): boolean => this.purchaseOrderService.comparePurchaseOrder(o1, o2);
  ...
  save(): void {
    ...
    const purchaseOrderItem =
      this.purchaseOrderItemFormService.getPurchaseOrderItem(this.editForm);
    if (purchaseOrderItem.id !== null) {
      this.subscribeToSaveResponse(
        this.purchaseOrderItemService.update(purchaseOrderItem),
      );
    } else {
      this.subscribeToSaveResponse(
        this.purchaseOrderItemService.create(purchaseOrderItem),
      );
    }
  }

  protected subscribeToSaveResponse(
    result: Observable<HttpResponse<IPurchaseOrderItem>>,
  ): void {
  ...
  protected updateForm(purchaseOrderItem: IPurchaseOrderItem): void {
    this.purchaseOrderItem = purchaseOrderItem;
    this.purchaseOrderItemFormService.resetForm(
      this.editForm,
      purchaseOrderItem,
    );
    ...
    this.purchaseOrderService
      ...
      .subscribe(
        (purchaseOrders: IPurchaseOrder[]) =>
          (this.purchaseOrdersSharedCollection = purchaseOrders),
      );
  }
}
