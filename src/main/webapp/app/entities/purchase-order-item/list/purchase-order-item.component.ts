...
import { EntityArrayResponseType, PurchaseOrderItemService } from '../service/purchase-order-item.service';
...
export class PurchaseOrderItemComponent implements OnInit {
  ...
  purchaseOrderItems = signal<IPurchaseOrderItem[]>([]);
  isLoading = false;
  ...
  protected readonly purchaseOrderItemService = inject(PurchaseOrderItemService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IPurchaseOrderItem): number => this.purchaseOrderItemService.getPurchaseOrderItemIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.purchaseOrderItems.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IPurchaseOrderItem[] | null): IPurchaseOrderItem[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.purchaseOrderItemService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
