...
export class SupplierService {
  ...
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/suppliers");
  ...
  update(supplier: ISupplier): Observable<EntityResponseType> {
    return this.http.put<ISupplier>(
      `${this.resourceUrl}/${this.getSupplierIdentifier(supplier)}`,
      supplier,
      { observe: "response" },
    );
  }
  ...
  partialUpdate(
    supplier: PartialUpdateSupplier,
  ): Observable<EntityResponseType> {
    return this.http.patch<ISupplier>(
      `${this.resourceUrl}/${this.getSupplierIdentifier(supplier)}`,
      supplier,
      { observe: "response" },
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<ISupplier>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  compareSupplier(
    o1: Pick<ISupplier, "id"> | null,
    o2: Pick<ISupplier, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getSupplierIdentifier(o1) === this.getSupplierIdentifier(o2)
      : o1 === o2;
  }
  ...
  addSupplierToCollectionIfMissing<Type extends Pick<ISupplier, "id">>(
    ...
    if (suppliers.length > 0) {
      const supplierCollectionIdentifiers = supplierCollection.map(
        (supplierItem) => this.getSupplierIdentifier(supplierItem),
      );
