...
export class ChapterService {
  ...
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/chapters");

  create(chapter: NewChapter): Observable<EntityResponseType> {
    return this.http.post<IChapter>(this.resourceUrl, chapter, {
      observe: "response",
    });
  }
  ...
  update(chapter: IChapter): Observable<EntityResponseType> {
    return this.http.put<IChapter>(
      `${this.resourceUrl}/${this.getChapterIdentifier(chapter)}`,
      chapter,
      { observe: "response" },
    );
  }
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IChapter>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IChapter[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  compareChapter(
    o1: Pick<IChapter, "id"> | null,
    o2: Pick<IChapter, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getChapterIdentifier(o1) === this.getChapterIdentifier(o2)
      : o1 === o2;
  }
  ...
  addChapterToCollectionIfMissing<Type extends Pick<IChapter, "id">>(
    ...
    if (chapters.length > 0) {
      const chapterCollectionIdentifiers = chapterCollection.map(
        (chapterItem) => this.getChapterIdentifier(chapterItem),
      );
