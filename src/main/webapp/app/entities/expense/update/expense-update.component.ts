...
export class ExpenseUpdateComponent implements OnInit {
  ...
  compareArticle = (o1: IArticle | null, o2: IArticle | null): boolean =>
    this.articleService.compareArticle(o1, o2);
  ...
  protected subscribeToSaveResponse(
    result: Observable<HttpResponse<IExpense>>,
  ): void {
  ...
  protected updateForm(expense: IExpense): void {
    ...
    this.articlesSharedCollection =
      this.articleService.addArticleToCollectionIfMissing<IArticle>(
        this.articlesSharedCollection,
        ...(expense.articles ?? []),
      );
  }
    ...
    this.annexDecisionService
      ...
      .pipe(
        map((annexDecisions: IAnnexDecision[]) =>
          this.annexDecisionService.addAnnexDecisionToCollectionIfMissing<IAnnexDecision>(
            annexDecisions,
            this.expense?.annexDecision,
          ),
        ),
      )
      .subscribe(
        (annexDecisions: IAnnexDecision[]) =>
          (this.annexDecisionsCollection = annexDecisions),
      );
    ...
    this.articleService
      ...
      .pipe(
        map((articles: IArticle[]) =>
          this.articleService.addArticleToCollectionIfMissing<IArticle>(
            articles,
            ...(this.expense?.articles ?? []),
          ),
        ),
      )
      .subscribe(
        (articles: IArticle[]) => (this.articlesSharedCollection = articles),
      );
  }
}
