...
export class ExpenseService {
  ...
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/expenses");

  create(expense: NewExpense): Observable<EntityResponseType> {
    return this.http.post<IExpense>(this.resourceUrl, expense, {
      observe: "response",
    });
  }
  ...
  update(expense: IExpense): Observable<EntityResponseType> {
    return this.http.put<IExpense>(
      `${this.resourceUrl}/${this.getExpenseIdentifier(expense)}`,
      expense,
      { observe: "response" },
    );
  }
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IExpense>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IExpense[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  compareExpense(
    o1: Pick<IExpense, "id"> | null,
    o2: Pick<IExpense, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getExpenseIdentifier(o1) === this.getExpenseIdentifier(o2)
      : o1 === o2;
  }
  ...
  addExpenseToCollectionIfMissing<Type extends Pick<IExpense, "id">>(
    ...
    if (expenses.length > 0) {
      const expenseCollectionIdentifiers = expenseCollection.map(
        (expenseItem) => this.getExpenseIdentifier(expenseItem),
      );
