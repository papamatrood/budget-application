...
export class MandateService {
  ...
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/mandates");
  ...
  update(mandate: IMandate): Observable<EntityResponseType> {
    ...
    return this.http
      .put<RestMandate>(
        `${this.resourceUrl}/${this.getMandateIdentifier(mandate)}`,
        copy,
        { observe: "response" },
      )
      .pipe(map((res) => this.convertResponseFromServer(res)));
  ...
  compareMandate(
    o1: Pick<IMandate, "id"> | null,
    o2: Pick<IMandate, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getMandateIdentifier(o1) === this.getMandateIdentifier(o2)
      : o1 === o2;
  }
  ...
  addMandateToCollectionIfMissing<Type extends Pick<IMandate, "id">>(
    ...
    if (mandates.length > 0) {
      const mandateCollectionIdentifiers = mandateCollection.map(
        (mandateItem) => this.getMandateIdentifier(mandateItem),
      );
  ...
  protected convertDateFromClient<
    T extends IMandate | NewMandate | PartialUpdateMandate,
  >(mandate: T): RestOf<T> {
    return {
      ...mandate,
  ...
  protected convertDateFromServer(restMandate: RestMandate): IMandate {
    return {
      ...restMandate,
      mandateDate: restMandate.mandateDate
        ? dayjs(restMandate.mandateDate)
        : undefined,
    };
  }

  protected convertResponseFromServer(
    res: HttpResponse<RestMandate>,
  ): HttpResponse<IMandate> {
    return res.clone({
