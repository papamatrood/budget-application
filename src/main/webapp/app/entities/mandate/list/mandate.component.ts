...
export class MandateComponent implements OnInit {
  ...
  protected readonly mandateService = inject(MandateService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IMandate): number =>
    this.mandateService.getMandateIdentifier(item);
  ...
  delete(mandate: IMandate): void {
    const modalRef = this.modalService.open(MandateDeleteDialogComponent, {
      size: "lg",
      backdrop: "static",
    });
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.mandates.set(dataFromBody);
  }
