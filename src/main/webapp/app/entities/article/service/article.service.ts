...
export class ArticleService {
  ...
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/articles");

  create(article: NewArticle): Observable<EntityResponseType> {
    return this.http.post<IArticle>(this.resourceUrl, article, {
      observe: "response",
    });
  }
  ...
  update(article: IArticle): Observable<EntityResponseType> {
    return this.http.put<IArticle>(
      `${this.resourceUrl}/${this.getArticleIdentifier(article)}`,
      article,
      { observe: "response" },
    );
  }
  ...
  compareArticle(
    o1: Pick<IArticle, "id"> | null,
    o2: Pick<IArticle, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getArticleIdentifier(o1) === this.getArticleIdentifier(o2)
      : o1 === o2;
  }
  ...
  addArticleToCollectionIfMissing<Type extends Pick<IArticle, "id">>(
    ...
    if (articles.length > 0) {
      const articleCollectionIdentifiers = articleCollection.map(
        (articleItem) => this.getArticleIdentifier(articleItem),
      );
