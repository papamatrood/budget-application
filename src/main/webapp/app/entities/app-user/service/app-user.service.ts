...
type RestOf<T extends IAppUser | NewAppUser> = Omit<
  T,
...
export class AppUserService {
  ...
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/app-users");
  ...
  updateUserAppUser(userAppUserDTO: IUserAppUserDTO): Observable<EntityResponseType> {
    ...
    return this.http
      .put<IUserAppUserDTO>(
        `${this.UserAppUserDTOresourceUrl}/${this.getUserAppUserDTOIdentifier(userAppUserDTO)}`,
        copy,
        { observe: "response" },
      )
      .pipe(map((res) => this.myConvertResponseFromServer(res)));
  ...
  update(appUser: IAppUser): Observable<EntityResponseType> {
    ...
    return this.http
      .put<RestAppUser>(
        `${this.resourceUrl}/${this.getAppUserIdentifier(appUser)}`,
        copy,
        { observe: "response" },
      )
      .pipe(map((res) => this.convertResponseFromServer(res)));
  ...
  compareAppUser(
    o1: Pick<IAppUser, "id"> | null,
    o2: Pick<IAppUser, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getAppUserIdentifier(o1) === this.getAppUserIdentifier(o2)
      : o1 === o2;
  }
  ...
  addAppUserToCollectionIfMissing<Type extends Pick<IAppUser, "id">>(
    ...
    if (appUsers.length > 0) {
      const appUserCollectionIdentifiers = appUserCollection.map(
        (appUserItem) => this.getAppUserIdentifier(appUserItem),
      );
  ...
  protected convertDateFromClient<
    T extends IAppUser | NewAppUser | PartialUpdateAppUser,
  >(appUser: T): RestOf<T> {
    return {
      ...appUser,
      lastDateUpdate: appUser.lastDateUpdate?.toJSON() ?? null,
      dateCreated: appUser.dateCreated?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restAppUser: RestAppUser): IAppUser {
    return {
      ...restAppUser,
      lastDateUpdate: restAppUser.lastDateUpdate
        ? dayjs(restAppUser.lastDateUpdate)
        : undefined,
      dateCreated: restAppUser.dateCreated
        ? dayjs(restAppUser.dateCreated)
        : undefined,
      birthDate: restAppUser.birthDate
        ? dayjs(restAppUser.birthDate)
        : undefined,
    };
  }

  protected convertResponseFromServer(
    res: HttpResponse<RestAppUser>,
  ): HttpResponse<IAppUser> {
    return res.clone({
  ...
  protected myConvertDateFromClient<T extends IUserAppUserDTO | NewUserAppUserDTO>(
    ...
    return {
      ...
      birthDate: dayjs.isDayjs(userAppUserDTO.birthDate)
        ? userAppUserDTO.birthDate.toJSON()
        : null,
    };
  }
