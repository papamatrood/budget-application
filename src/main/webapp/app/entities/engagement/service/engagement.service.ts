...
export type PartialUpdateEngagement = Partial<IEngagement> &
  Pick<IEngagement, "id">;

type RestOf<T extends IEngagement | NewEngagement> = Omit<
  T,
  "engagementDate"
> & {
...
export class EngagementService {
  ...
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/engagements");
  ...
  update(engagement: IEngagement): Observable<EntityResponseType> {
    ...
    return this.http
      .put<RestEngagement>(
        `${this.resourceUrl}/${this.getEngagementIdentifier(engagement)}`,
        copy,
        { observe: "response" },
      )
      .pipe(map((res) => this.convertResponseFromServer(res)));
  ...
  partialUpdate(
    engagement: PartialUpdateEngagement,
  ): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(engagement);
    return this.http
      .patch<RestEngagement>(
        `${this.resourceUrl}/${this.getEngagementIdentifier(engagement)}`,
        copy,
        { observe: "response" },
      )
  ...
  compareEngagement(
    o1: Pick<IEngagement, "id"> | null,
    o2: Pick<IEngagement, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getEngagementIdentifier(o1) === this.getEngagementIdentifier(o2)
      : o1 === o2;
  }
  ...
  addEngagementToCollectionIfMissing<Type extends Pick<IEngagement, "id">>(
    ...
    if (engagements.length > 0) {
      const engagementCollectionIdentifiers = engagementCollection.map(
        (engagementItem) => this.getEngagementIdentifier(engagementItem),
      );
      const engagementsToAdd = engagements.filter((engagementItem) => {
        const engagementIdentifier =
          this.getEngagementIdentifier(engagementItem);
  ...
  protected convertDateFromClient<
    T extends IEngagement | NewEngagement | PartialUpdateEngagement,
  >(engagement: T): RestOf<T> {
    return {
      ...engagement,
  ...
  protected convertDateFromServer(restEngagement: RestEngagement): IEngagement {
    return {
      ...restEngagement,
      engagementDate: restEngagement.engagementDate
        ? dayjs(restEngagement.engagementDate)
        : undefined,
    };
  }

  protected convertResponseFromServer(
    res: HttpResponse<RestEngagement>,
  ): HttpResponse<IEngagement> {
    return res.clone({
