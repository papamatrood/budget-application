...
export type PartialUpdateFinancialYear = Partial<IFinancialYear> &
  Pick<IFinancialYear, "id">;
...
export class FinancialYearService {
  ...
  create(financialYear: NewFinancialYear): Observable<EntityResponseType> {
    return this.http.post<IFinancialYear>(this.resourceUrl, financialYear, {
      observe: "response",
    });
  }

  update(financialYear: IFinancialYear): Observable<EntityResponseType> {
    return this.http.put<IFinancialYear>(
      `${this.resourceUrl}/${this.getFinancialYearIdentifier(financialYear)}`,
      financialYear,
      {
        observe: "response",
      },
  ...
  partialUpdate(
    financialYear: PartialUpdateFinancialYear,
  ): Observable<EntityResponseType> {
    return this.http.patch<IFinancialYear>(
      `${this.resourceUrl}/${this.getFinancialYearIdentifier(financialYear)}`,
      financialYear,
      {
        observe: "response",
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IFinancialYear>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IFinancialYear[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  getFinancialYearIdentifier(
    financialYear: Pick<IFinancialYear, "id">,
  ): number {
    return financialYear.id;
  }

  compareFinancialYear(
    o1: Pick<IFinancialYear, "id"> | null,
    o2: Pick<IFinancialYear, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getFinancialYearIdentifier(o1) ===
          this.getFinancialYearIdentifier(o2)
      : o1 === o2;
  }

  addFinancialYearToCollectionIfMissing<
    Type extends Pick<IFinancialYear, "id">,
  >(
    ...
    if (financialYears.length > 0) {
      const financialYearCollectionIdentifiers = financialYearCollection.map(
        (financialYearItem) =>
          this.getFinancialYearIdentifier(financialYearItem),
      );
      const financialYearsToAdd = financialYears.filter((financialYearItem) => {
        const financialYearIdentifier =
          this.getFinancialYearIdentifier(financialYearItem);
        if (
          financialYearCollectionIdentifiers.includes(financialYearIdentifier)
        ) {
          return false;
        }
