...
export class PurchaseOrderUpdateComponent implements OnInit {
  ...
  editForm: PurchaseOrderFormGroup =
    this.purchaseOrderFormService.createPurchaseOrderFormGroup();
  ...
  compareSupplier = (o1: ISupplier | null, o2: ISupplier | null): boolean =>
    this.supplierService.compareSupplier(o1, o2);
  ...
  save(): void {
    this.isSaving = true;
    const purchaseOrder = this.purchaseOrderFormService.getPurchaseOrder(
      this.editForm,
    );
  ...
  protected subscribeToSaveResponse(
    result: Observable<HttpResponse<IPurchaseOrder>>,
  ): void {
  ...
  protected updateForm(purchaseOrder: IPurchaseOrder): void {
    ...
    this.suppliersSharedCollection =
      this.supplierService.addSupplierToCollectionIfMissing<ISupplier>(
        this.suppliersSharedCollection,
    ...
    this.supplierService
      ...
      .pipe(
        map((suppliers: ISupplier[]) =>
          this.supplierService.addSupplierToCollectionIfMissing<ISupplier>(
            suppliers,
            this.purchaseOrder?.supplier,
          ),
        ),
      )
      .subscribe(
        (suppliers: ISupplier[]) =>
          (this.suppliersSharedCollection = suppliers),
      );
    ...
    this.engagementService
      ...
      .pipe(
        map((engagements: IEngagement[]) =>
          this.engagementService.addEngagementToCollectionIfMissing<IEngagement>(
            engagements,
            this.purchaseOrder?.engagement,
          ),
        ),
      )
      .subscribe(
        (engagements: IEngagement[]) =>
          (this.engagementsSharedCollection = engagements),
      );
  }
}
