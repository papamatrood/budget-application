...
export type PartialUpdatePurchaseOrder = Partial<IPurchaseOrder> &
  Pick<IPurchaseOrder, "id">;

type RestOf<T extends IPurchaseOrder | NewPurchaseOrder> = Omit<
  T,
  "orderDate"
> & {
...
export class PurchaseOrderService {
  ...
  update(purchaseOrder: IPurchaseOrder): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(purchaseOrder);
    return this.http
      .put<RestPurchaseOrder>(
        `${this.resourceUrl}/${this.getPurchaseOrderIdentifier(purchaseOrder)}`,
        copy,
        { observe: "response" },
      )
      .pipe(map((res) => this.convertResponseFromServer(res)));
  ...
  partialUpdate(
    purchaseOrder: PartialUpdatePurchaseOrder,
  ): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(purchaseOrder);
    return this.http
      .patch<RestPurchaseOrder>(
        `${this.resourceUrl}/${this.getPurchaseOrderIdentifier(purchaseOrder)}`,
        copy,
        { observe: "response" },
      )
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestPurchaseOrder>(`${this.resourceUrl}/${id}`, {
        observe: "response",
      })
      .pipe(map((res) => this.convertResponseFromServer(res)));
  }
  ...
  getPurchaseOrderIdentifier(
    purchaseOrder: Pick<IPurchaseOrder, "id">,
  ): number {
    return purchaseOrder.id;
  }

  comparePurchaseOrder(
    o1: Pick<IPurchaseOrder, "id"> | null,
    o2: Pick<IPurchaseOrder, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getPurchaseOrderIdentifier(o1) ===
          this.getPurchaseOrderIdentifier(o2)
      : o1 === o2;
  }

  addPurchaseOrderToCollectionIfMissing<
    Type extends Pick<IPurchaseOrder, "id">,
  >(
    ...
    if (purchaseOrders.length > 0) {
      ...
      const purchaseOrdersToAdd = purchaseOrders.filter((purchaseOrderItem) => {
        const purchaseOrderIdentifier =
          this.getPurchaseOrderIdentifier(purchaseOrderItem);
        if (
          purchaseOrderCollectionIdentifiers.includes(purchaseOrderIdentifier)
        ) {
          return false;
        }
  ...
  protected convertDateFromClient<
    T extends IPurchaseOrder | NewPurchaseOrder | PartialUpdatePurchaseOrder,
  >(purchaseOrder: T): RestOf<T> {
    return {
      ...purchaseOrder,
  ...
  protected convertDateFromServer(
    restPurchaseOrder: RestPurchaseOrder,
  ): IPurchaseOrder {
    return {
      ...restPurchaseOrder,
      orderDate: restPurchaseOrder.orderDate
        ? dayjs(restPurchaseOrder.orderDate)
        : undefined,
    };
  }

  protected convertResponseFromServer(
    res: HttpResponse<RestPurchaseOrder>,
  ): HttpResponse<IPurchaseOrder> {
    return res.clone({
