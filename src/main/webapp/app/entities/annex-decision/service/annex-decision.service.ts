...
export type PartialUpdateAnnexDecision = Partial<IAnnexDecision> &
  Pick<IAnnexDecision, "id">;
...
export class AnnexDecisionService {
  ...
  create(annexDecision: NewAnnexDecision): Observable<EntityResponseType> {
    return this.http.post<IAnnexDecision>(this.resourceUrl, annexDecision, {
      observe: "response",
    });
  }

  update(annexDecision: IAnnexDecision): Observable<EntityResponseType> {
    return this.http.put<IAnnexDecision>(
      `${this.resourceUrl}/${this.getAnnexDecisionIdentifier(annexDecision)}`,
      annexDecision,
      {
        observe: "response",
      },
  ...
  partialUpdate(
    annexDecision: PartialUpdateAnnexDecision,
  ): Observable<EntityResponseType> {
    return this.http.patch<IAnnexDecision>(
      `${this.resourceUrl}/${this.getAnnexDecisionIdentifier(annexDecision)}`,
      annexDecision,
      {
        observe: "response",
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IAnnexDecision>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IAnnexDecision[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  getAnnexDecisionIdentifier(
    annexDecision: Pick<IAnnexDecision, "id">,
  ): number {
    return annexDecision.id;
  }

  compareAnnexDecision(
    o1: Pick<IAnnexDecision, "id"> | null,
    o2: Pick<IAnnexDecision, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getAnnexDecisionIdentifier(o1) ===
          this.getAnnexDecisionIdentifier(o2)
      : o1 === o2;
  }

  addAnnexDecisionToCollectionIfMissing<
    Type extends Pick<IAnnexDecision, "id">,
  >(
    ...
    if (annexDecisions.length > 0) {
      const annexDecisionCollectionIdentifiers = annexDecisionCollection.map(
        (annexDecisionItem) =>
          this.getAnnexDecisionIdentifier(annexDecisionItem),
      );
      const annexDecisionsToAdd = annexDecisions.filter((annexDecisionItem) => {
        const annexDecisionIdentifier =
          this.getAnnexDecisionIdentifier(annexDecisionItem);
        if (
          annexDecisionCollectionIdentifiers.includes(annexDecisionIdentifier)
        ) {
          return false;
        }
