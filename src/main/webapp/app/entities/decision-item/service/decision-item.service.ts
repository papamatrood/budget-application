...
export type PartialUpdateDecisionItem = Partial<IDecisionItem> &
  Pick<IDecisionItem, "id">;
...
export class DecisionItemService {
  ...
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/decision-items");

  create(decisionItem: NewDecisionItem): Observable<EntityResponseType> {
    return this.http.post<IDecisionItem>(this.resourceUrl, decisionItem, {
      observe: "response",
    });
  }

  update(decisionItem: IDecisionItem): Observable<EntityResponseType> {
    return this.http.put<IDecisionItem>(
      `${this.resourceUrl}/${this.getDecisionItemIdentifier(decisionItem)}`,
      decisionItem,
      {
        observe: "response",
      },
  ...
  partialUpdate(
    decisionItem: PartialUpdateDecisionItem,
  ): Observable<EntityResponseType> {
    return this.http.patch<IDecisionItem>(
      `${this.resourceUrl}/${this.getDecisionItemIdentifier(decisionItem)}`,
      decisionItem,
      {
        observe: "response",
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IDecisionItem>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IDecisionItem[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  compareDecisionItem(
    o1: Pick<IDecisionItem, "id"> | null,
    o2: Pick<IDecisionItem, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getDecisionItemIdentifier(o1) ===
          this.getDecisionItemIdentifier(o2)
      : o1 === o2;
  }
  ...
  addDecisionItemToCollectionIfMissing<Type extends Pick<IDecisionItem, "id">>(
    ...
    if (decisionItems.length > 0) {
      const decisionItemCollectionIdentifiers = decisionItemCollection.map(
        (decisionItemItem) => this.getDecisionItemIdentifier(decisionItemItem),
      );
      const decisionItemsToAdd = decisionItems.filter((decisionItemItem) => {
        const decisionItemIdentifier =
          this.getDecisionItemIdentifier(decisionItemItem);
        if (
          decisionItemCollectionIdentifiers.includes(decisionItemIdentifier)
        ) {
          return false;
        }
