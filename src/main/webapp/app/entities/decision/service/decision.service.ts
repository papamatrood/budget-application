...
export class DecisionService {
  ...
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/decisions");
  ...
  update(decision: IDecision): Observable<EntityResponseType> {
    ...
    return this.http
      .put<RestDecision>(
        `${this.resourceUrl}/${this.getDecisionIdentifier(decision)}`,
        copy,
        { observe: "response" },
      )
      .pipe(map((res) => this.convertResponseFromServer(res)));
  ...
  partialUpdate(
    decision: PartialUpdateDecision,
  ...
  compareDecision(
    o1: Pick<IDecision, "id"> | null,
    o2: Pick<IDecision, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getDecisionIdentifier(o1) === this.getDecisionIdentifier(o2)
      : o1 === o2;
  }
  ...
  addDecisionToCollectionIfMissing<Type extends Pick<IDecision, "id">>(
    ...
    if (decisions.length > 0) {
      const decisionCollectionIdentifiers = decisionCollection.map(
        (decisionItem) => this.getDecisionIdentifier(decisionItem),
      );
  ...
  protected convertDateFromClient<
    T extends IDecision | NewDecision | PartialUpdateDecision,
  >(decision: T): RestOf<T> {
    return {
      ...decision,
  ...
  protected convertDateFromServer(restDecision: RestDecision): IDecision {
    return {
      ...restDecision,
      decisionDate: restDecision.decisionDate
        ? dayjs(restDecision.decisionDate)
        : undefined,
    };
  }

  protected convertResponseFromServer(
    res: HttpResponse<RestDecision>,
  ): HttpResponse<IDecision> {
    return res.clone({
