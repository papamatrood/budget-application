...
export class RecipeService {
  ...
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/recipes");

  create(recipe: NewRecipe): Observable<EntityResponseType> {
    return this.http.post<IRecipe>(this.resourceUrl, recipe, {
      observe: "response",
    });
  }
  ...
  update(recipe: IRecipe): Observable<EntityResponseType> {
    return this.http.put<IRecipe>(
      `${this.resourceUrl}/${this.getRecipeIdentifier(recipe)}`,
      recipe,
      { observe: "response" },
    );
  }
  ...
  compareRecipe(
    o1: Pick<IRecipe, "id"> | null,
    o2: Pick<IRecipe, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getRecipeIdentifier(o1) === this.getRecipeIdentifier(o2)
      : o1 === o2;
  }
  ...
  addRecipeToCollectionIfMissing<Type extends Pick<IRecipe, "id">>(
    ...
    if (recipes.length > 0) {
      const recipeCollectionIdentifiers = recipeCollection.map((recipeItem) =>
        this.getRecipeIdentifier(recipeItem),
      );
