...
import { FinancialYearDeleteDialogComponent } from "../delete/financial-year-delete-dialog.component";
...
import { EntityArrayResponseType } from "app/entities/user/service/user.service";
import { FinancialYearUpdateComponent } from "../update/financial-year-update.component";

@Component({
  selector: "jhi-financial-year",
...
export class FinancialYearComponent implements OnInit {
  ...
  financialYears = signal<IFinancialYear[]>([]);
  isLoading = false;
  ...
  protected readonly financialYearService = inject(FinancialYearService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IFinancialYear): number =>
    this.financialYearService.getFinancialYearIdentifier(item);
  ...
  openCreateModal(): void {
    const modalRef = this.modalService.open(FinancialYearUpdateComponent, {
      size: "lg",
      backdrop: "static",
    });
    modalRef.componentInstance.financialYear = undefined; // Pour la création
    modalRef.closed
      .pipe(
  ...
  openEditModal(financialYear: IFinancialYear): void {
    const modalRef = this.modalService.open(FinancialYearUpdateComponent, {
      size: "lg",
      backdrop: "static",
    });
    modalRef.componentInstance.financialYear = financialYear; // Envoie l'objet complet au modal
    modalRef.closed
      .pipe(
  ...
  delete(financialYear: IFinancialYear): void {
    const modalRef = this.modalService.open(
      FinancialYearDeleteDialogComponent,
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.financialYears.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(
    data: IFinancialYear[] | null,
  ): IFinancialYear[] {
    return data ?? [];
  }
