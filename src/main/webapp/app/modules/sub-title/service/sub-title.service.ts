...
import { ISubTitle, NewSubTitle } from "app/entities/sub-title/sub-title.model";
...
export class SubTitleService {
  ...
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/sub-titles");

  create(subTitle: NewSubTitle): Observable<EntityResponseType> {
    return this.http.post<ISubTitle>(this.resourceUrl, subTitle, {
      observe: "response",
    });
  }

  update(subTitle: ISubTitle): Observable<EntityResponseType> {
    return this.http.put<ISubTitle>(
      `${this.resourceUrl}/${this.getSubTitleIdentifier(subTitle)}`,
      subTitle,
      { observe: "response" },
    );
  }
  ...
  partialUpdate(
    subTitle: PartialUpdateSubTitle,
  ): Observable<EntityResponseType> {
    return this.http.patch<ISubTitle>(
      `${this.resourceUrl}/${this.getSubTitleIdentifier(subTitle)}`,
      subTitle,
      { observe: "response" },
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<ISubTitle>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<ISubTitle[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  compareSubTitle(
    o1: Pick<ISubTitle, "id"> | null,
    o2: Pick<ISubTitle, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getSubTitleIdentifier(o1) === this.getSubTitleIdentifier(o2)
      : o1 === o2;
  }
  ...
  addSubTitleToCollectionIfMissing<Type extends Pick<ISubTitle, "id">>(
    ...
    if (subTitles.length > 0) {
      const subTitleCollectionIdentifiers = subTitleCollection.map(
        (subTitleItem) => this.getSubTitleIdentifier(subTitleItem),
      );
